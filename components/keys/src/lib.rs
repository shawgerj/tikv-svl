...
// for wotr gc, we ignore the raft_group_id part of the raft
// we just care that it is actually a raft key
pub fn is_raft_key(key: &[u8]) -> bool {
    let suffix_idx = REGION_RAFT_PREFIX_KEY.len() + mem::size_of::<u64>();
    let expect_key_len = suffix_idx + mem::size_of::<u8>() + mem::size_of::<u64>();
    return key.len() == expect_key_len
